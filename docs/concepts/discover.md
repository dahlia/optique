...
Generated modules include a fixed machine-owned header with lint, formatter,
and TypeScript check suppression comments for tools that support file-level
comments.  The module map imports command files as static TypeScript
specifier strings, but command module exports are still validated at runtime by
`commandsFromModules()`.
